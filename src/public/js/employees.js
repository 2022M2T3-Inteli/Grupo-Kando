    ...
    tableData[index].tools = `
			<div class="employee-tools">
				...
				<div class="material-symbols-outlined employee-view-button" 
					...
					onclick="viewEmployeeInfo(${index}, ${row.id})">
						visibility
				</div>
    ...
    tableData[index].tools = `
			<div class="employee-tools">
				...
				<div class="material-symbols-outlined employee-view-button" 
					data-bs-toggle="modal"
					data-bs-target="#view-employee-modal"
					onclick="viewEmployeeInfo(${index})">
						visibility
				</div>
...
let projectsAssignedTable = $('#projects-assigned-table')
let projectsAssignedData = []

function viewProjectsAssigned(id) {
  let url = 'employees/projectsassigned/' + id
  ...
  data.forEach(row => {
    projectsAssignedData.push(row)
  })

  console.log(projectsAssignedData)

  $(projectsAssignedTable).bootstrapTable('destroy')
  setTimeout(function () {
    $(projectsAssignedTable).bootstrapTable({
      data: projectsAssignedData
    })
    // $('#employees-assigned-table tr:not(:first)').addClass('table-body-row')
  }, 0)
}

function getHoursProjectAssigned(employee_id, project_id) {
  let url = `employees/hoursprojectsassigned/${employee_id}/${project_id}`
...
// getHoursProjectAssigned(2, 14)

function viewEmployeeInfo(index, id) {
  viewEmployee(index)
  viewProjectsAssigned(id)
}
